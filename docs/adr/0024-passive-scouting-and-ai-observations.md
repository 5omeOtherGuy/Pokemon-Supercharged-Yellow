# ADR-0024: Disclose active passives and bound AI observations

- Status: Accepted
- Date: 2026-09-05
- Decision authority: Explicit user acceptance of the reciprocal active-effect disclosure recommendation on 2026-09-05
...
That tentative suggestion did not override [ADR-0009](0009-vgc-style-ai-information.md), which protects the player's exact stats, allocations, pending actions and future random outcomes. [ADR-0007](0007-full-boss-scouting.md) already accepts full boss team, moves, abilities, held items and bag-supply scouting. The user subsequently accepted reciprocal disclosure of active trainer passives and assigned Pokémon capabilities while retaining those information limits.
...
- The AI may inspect its own team state. Player access to exact boss numerical stats remains an open scouting question under ADR-0007; this decision does not settle it.
...
Use legal-action handling and defined behavior for unknown information; do not treat unrestricted access as a substitute for correct mechanics integration. Source-specific feasibility and every data path require an audit after engine selection. This engineering approach supports the accepted information policy; no concrete implementation or runtime behavior has been verified.
...
The initial user statement on 2026-09-05 established preparation and public gym-leader information as goals, with a tentative AI-access suggestion to avoid glitches. At that point reciprocal active-effect disclosure remained proposed. In the subsequent review discussion, after the recommendation was restated with hidden-stat and pending-action protections, the user explicitly answered: "In regards to (2) we accept it as per your recommendation." This accepts the reciprocal active-effect fields and preserves the existing information boundary.
...
- The AI evaluates custom effects using the accepted visible fields.
...
Map the accepted fields into the selected engine and prototype a boss with both passive systems when implementation is authorised. Preserve existing accepted information limits.

## Evidence review — 2026-09-05

The documentation audit rechecked `IsAiBattlerAware` (lines 200–206) and `AI_DecideKnownAbilityForTurn` (lines 1760–1805) at the pinned revision above. Their awareness and candidate-ability branches support only the stated example of knowledge modelling. They do not establish end-to-end isolation of hidden data or custom-effect support. Status remained Proposed at audit completion; no user uncertainty was treated as acceptance.

## Acceptance — 2026-09-05, review follow-up

The user's explicit acceptance above changes this record from Proposed to Accepted. Disclosure covers active trainer builds and assigned Pokémon capabilities on both sides of boss battles, with precise effect descriptions. Inactive collections, exact player stats/training, committed actions and future random outcomes remain protected. Story presentation and implementation details remain open; acceptance is not runtime validation.
