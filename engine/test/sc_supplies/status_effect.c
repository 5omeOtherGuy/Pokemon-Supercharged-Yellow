...
TEST("SC supplies: Antidote does not clear confusion or infatuation")
{
    ...
    EXPECT_EQ(gBattleMons[0].volatiles.confusionTimer, 3);
    EXPECT_EQ(gBattleMons[0].volatiles.infatuation, 2);
}
...
TEST("SC supplies: Full Heal clears both confusion and infatuation")
{
    ...
    EXPECT_EQ(gBattleMons[0].volatiles.confusionTimer, 0);
    EXPECT_EQ(gBattleMons[0].volatiles.infatuation, 0);
}
