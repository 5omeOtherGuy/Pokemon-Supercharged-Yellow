...
sc_output=$(mktemp -d /tmp/sc-field-objects-XXXXXX)
trap 'rm -rf "$sc_output"' EXIT
cd "$sc_generated_engine"
for sc_source in sc_field party_menu field_player_avatar scrcmd pokemon_storage_system region_map; do
  ...
  arm-none-eabi-gcc -c -x cpp-output "$sc_output/$sc_source.i" -o "$sc_output/$sc_source.o" \
    ...
    -std=gnu17 -Werror -Wall -Wno-pointer-to-int-cast -Wno-strict-aliasing \
