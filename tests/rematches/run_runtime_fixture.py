"""Ordinary-input logged diagnostic session; each command has one response.

The separate ROM is explicitly fixture-patched; runtime memory writes and
savestates are unavailable in the required ordinary headless driver.
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--driver',type=Path,required=True);ap.add_argument('--rom',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);ap.add_argument('--commands',type=Path,help='Replay a complete recorded trace instead of bootstrapping interactively');args=ap.parse_args()
    ...
    if not driver.exists():ap.error('ordinary tools/emulator/headless.c driver must exist')
    ...
    with (out/'commands.txt').open('w',buffering=1) as commands,(out/'responses.jsonl').open('w',buffering=1) as responses:
        ...
        def send(command):
            if command.split()[0] not in ('run','release','reset','status','read','screenshot','quit'):
                raise ValueError('Only ordinary inputs and observations are allowed')
